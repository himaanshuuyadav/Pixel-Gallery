...
/**
 * Pair of light and dark color schemes generated from a seed color
 */
...
/**
 * Extracts a vibrant seed color from a bitmap using Palette API
...
fun extractSeedColor(bitmap: Bitmap): Color {
    ...
    val color = palette.vibrantSwatch?.rgb?.let { Color(it) }
        ...
        ?: Color(0xFF6C4FF5) // Fallback to our gallery primary color
...
/**
 * Generates a complete Material 3 color scheme pair (light + dark) from a seed color
 * Uses HCT color space for perceptually accurate color manipulation
 * Intelligently handles grayscale images to preserve neutrality
 */
