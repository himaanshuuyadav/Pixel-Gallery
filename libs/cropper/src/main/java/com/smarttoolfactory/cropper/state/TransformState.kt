...
open class TransformState(
    ...
    internal var initialOffsetY: Int = 0,
    ...
    var drawAreaRect: Rect by mutableStateOf(
        Rect(
            offset = Offset(
                ...
                y = ((containerSize.height - drawAreaSize.height) / 2).toFloat() + initialOffsetY
            ),
    ...
    internal val animatablePanX = Animatable(initialPanX)
    internal val animatablePanY = Animatable(initialPanY)
    ...
    internal open suspend fun updateTransformState(
        ...
        if (pannable) {
            ...
            snapPanXto(newPan.x)
            // Compensate for the visual shift caused by scaling an image offset by initialOffsetY
            val panYComp = initialOffsetY * (newZoom - this.zoom)
            snapPanYto(newPan.y - panYComp)
        }
    }
