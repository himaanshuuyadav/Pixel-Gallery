...
class DynamicCropState internal constructor(
    ...
    override suspend fun onMove(changes: List<PointerInputChange>) {
        ...
        gestureInvoked = false // Disabled resizing frame on pinch
    ...
    override suspend fun onGesture(
        ...
        } else if (touchRegion == TouchRegion.None || changes.size > 1) {
            doubleTapped = false
    ...
    private suspend fun animateResizeToFitOverlay() {
        ...
        val newPanY = newDrawArea.center.y - containerHeight / 2f - initialOffsetY * newZoom
    ...
    override suspend fun onDoubleTap(
        ...
        val panYComp = initialOffsetY * (zoom - this.zoom)
        val newPan = Offset(pan.x, pan.y - panYComp)
        resetWithAnimation(pan = newPan, zoom = zoom, rotation = rotation)
...
//    }
    ...
    private fun updateOverlayRect(
        ...
        val screenPositionX = (position.x + distanceToEdgeFromTouch.x).coerceIn(drawAreaRect.left, drawAreaRect.right)
        val screenPositionY = (position.y + distanceToEdgeFromTouch.y).coerceIn(drawAreaRect.top, drawAreaRect.bottom)
        ...
        return when (touchRegion) {
            ...
            TouchRegion.Inside -> {
                ...
                val translatedRect = overlayRect.translate(drag.x, drag.y)
                
                // Restrict translatedRect within drawAreaRect bounds
                val dx = if (translatedRect.left < drawAreaRect.left) drawAreaRect.left - translatedRect.left
                         else if (translatedRect.right > drawAreaRect.right) drawAreaRect.right - translatedRect.right
                         else 0f
                val dy = if (translatedRect.top < drawAreaRect.top) drawAreaRect.top - translatedRect.top
                         else if (translatedRect.bottom > drawAreaRect.bottom) drawAreaRect.bottom - translatedRect.bottom
                         else 0f
    ...
    private fun getTouchRegionInternal(
        ...
        if (rect.contains(position)) {
            return TouchRegion.None // Changed from Inside to None to pan image instead of moving frame
        }
    ...
    private fun getDistanceToEdgeFromTouch(
        ...
        TouchRegion.LeftEdge -> {
            Offset(rect.left - touchPosition.x, 0f)
        }

        TouchRegion.RightEdge -> {
            Offset(rect.right - touchPosition.x, 0f)
        }

        TouchRegion.TopEdge -> {
            Offset(0f, rect.top - touchPosition.y)
        }

        TouchRegion.BottomEdge -> {
            Offset(0f, rect.bottom - touchPosition.y)
        }
